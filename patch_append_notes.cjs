...
// We will insert `appendNotesChapterToFeed` right before `function openNotesSidebar`
const targetPoint = `      function openNotesSidebar`;

const newFunc = `      function appendNotesChapterToFeed(book, chapter, prepend = false) {
...
if (code.includes(targetPoint)) {
    code = code.replace(targetPoint, newFunc);
